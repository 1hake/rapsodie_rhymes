...
function tabMatcher(word, realWord) {
  if (rhymeTabs[word[0]]) {
    const patternObject = rhymeTabs[word[0]].filter(rhyme => {
      return (
        word.substring(0, rhyme.pattern.length).includes(rhyme.pattern) &&
        rhyme.pattern
      );
    });
    if (patternObject.length === 1) {
      return patternObject[0].rule
        ? applyRule(patternObject[0], word, realWord)
        : patternObject[0];
    } else if (patternObject.length > 1) {
      var chosenPattern = patternObject[0];
      patternObject.map(object => {
        if (
          object.pattern.split("").length >
          chosenPattern.pattern.split("").length
        ) {
